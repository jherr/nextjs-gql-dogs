...
export type QueryDogsArgs = {
  maxAgeInWeeks?: InputMaybe<Scalars['Float']>;
  maxWeight?: InputMaybe<Scalars['Float']>;
  minAgeInWeeks?: InputMaybe<Scalars['Float']>;
...
export const GetDogsDocument = gql`
    query getDogs($minWeight: Float, $maxWeight: Float, $sex: String, $minAgeInWeeks: Float, $maxAgeInWeeks: Float) {
  dogs(
    minWeight: $minWeight
    maxWeight: $maxWeight
    sex: $sex
    minAgeInWeeks: $minAgeInWeeks
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    dogByName(variables: DogByNameQueryVariables, requestHeaders?: Dom.RequestInit["headers"]): Promise<DogByNameQuery> {
      return withWrapper((wrappedRequestHeaders) => client.request<DogByNameQuery>(DogByNameDocument, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'dogByName');
    },
    getDogs(variables?: GetDogsQueryVariables, requestHeaders?: Dom.RequestInit["headers"]): Promise<GetDogsQuery> {
      return withWrapper((wrappedRequestHeaders) => client.request<GetDogsQuery>(GetDogsDocument, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'getDogs');
    }
  };
}
